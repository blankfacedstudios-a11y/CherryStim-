...
| Lint | `npm run lint` |
| Test | `npm test` (Vitest, 115 tests) |
...
- **Test suite**: Vitest + React Testing Library + jsdom. Tests live in `__tests__/` mirroring the source tree. Run with `npm test`. The `livekit-server-sdk` module is mocked in tests (its `AccessToken.toJwt()` fails in jsdom); see `__tests__/lib/livekit.test.ts` for the mock pattern. Components using `next/image` and `next/navigation` are mocked in their test files.
